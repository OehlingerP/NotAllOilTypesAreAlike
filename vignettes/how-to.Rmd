...
The goal of this vignette is to approximate the welfare losses of one specific
scenario. For this example, we first estimate the elasticities of substitution 
for Germany and then approximate the welfare loss for the scenario where 
...
estimated. This is the case when less then three varieties are available. In 
this case `sigma` cannot be estimated using Feenstra's method.

## Approximation of welfare loss
...
`est_welfare` estimates the loss in overall production from a CES and a nested
...
`r df_welfare[1,1] - df_welfare[1,2]` higher than using a standard CES production 
function.
